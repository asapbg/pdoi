                            ...
                            <div class="col-md-2 col-12">
                                <div class="form-group">
                                    <label class="col-sm-12 control-label" for="active">
                                        {{ __('validation.attributes.user_legal_form') }} <span class="required">*</span>
                                    </label>
                                    <div class="col-12">
                                        <select id="user_legal_form" name="user_legal_form"  class="form-control form-control-sm @error('user_legal_form'){{ 'is-invalid' }}@enderror">
                                            @if(!$item->id)
                                                <option value="">---</option>
                                            @endif
                                            @foreach(\App\Models\User::getUserLegalForms() as $val => $name)
                                                <option value="{{ $val }}" @if(old('user_legal_form', ($item->id ? $item->active : 1)) == $val) selected @endif>{{ $name }}</option>
                                            @endforeach
                                        </select>
                                        @error('user_legal_form')
